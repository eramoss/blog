...
function PostList({ posts }: Props) {
	...
	const [filterDates, setFilterDates] = React.useState<[string, string] | null>(
	...
	const handleDateFilter = (start: string, end: string) => {
		...
		if (start && end) {
			...
			if (startDate > endDate) {
				const randomMessage =
					SASSY_MESSAGES[Math.floor(Math.random() * SASSY_MESSAGES.length)];
	...
	filteredPosts = filteredPosts.filter(
		(post) =>
			!filterDates ||
			((!filterDates[0] ||
				new Date(post.date || "2000-10-10").getTime() >=
				new Date(filterDates[0]).getTime()) &&
				(!filterDates[1] ||
					new Date(post.date || "2000-10-10").getTime() <=
					new Date(filterDates[1]).getTime())),
	);

	filteredPosts = filteredPosts.sort(
		(a, b) =>
			new Date(b.date || "2000-10-10").getTime() -
			new Date(a.date || "2000-10-10").getTime(),
	);
				...
				<div className="pt-32 pb-12 md:pt-40 md:pb-20">
					<div className="max-w-3xl pb-12 md:pb-20 text-center md:text-left">
						...
						<p className="text-xl text-gray-600 dark:text-gray-400">
							I write about things that interest me, and sometimes I even finish
							them.
						</p>
					</div>
						...
						<div className="md:grow -mt-4">
							{filteredPosts.length > 0 ? (
								...
								<p className="text-gray-500 mt-8">
									Nenhum post encontrado para essa loucura de filtros.
								</p>
							)}
						</div>
					</div>
